...
<section class="row" style="width: 60%;margin: 0 auto;height: 80%">
    ...
    <div class="col-md-8 col-md-offset-2" style="margin-top: -300px">
        <form action="UpdateMovieTypeAction.action" class="fh5co-form animate-box" data-animate-effect="fadeInRight" method="post" style="border-radius: 20px;margin-top: 20px;">
            <%
                LookMovieTypeDao lookMovieTypeDao=new LookMovieTypeDaoImpl();
                String type=request.getParameter("type");
                System.out.println(type);
                List<MovieType> list=lookMovieTypeDao.searchMovieType(type);
                for (MovieType MovieType : list) {
            %>
            <input name="id" style="display: none" value="<%=MovieType.getType()%>">
            <div class="form-group">
                <label for="type" class="sr-only">电影类型</label>
                <input type="text" class="form-control" id="type" name="type" value="<%=MovieType.getType()%>">
            </div>
